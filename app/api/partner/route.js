  ...
  try {
    ...
    if (checkRes.data.status === 0) {
      ...
      if (saveRes.data.success) {
        ...
        return new Response(
          JSON.stringify({ status: true, code: emailCode, addlead: addleadRes.data }),
          { status: 201 }
        );
      } else {
        return new Response(
          JSON.stringify({ status: false, result: saveRes.data, error: "Failed to save partner" }),
          { status: 201 }
        );
      }
    } else {
      return new Response(JSON.stringify({ status: false, error: "Email already exists" }), {
        status: 201,
      });
    }
  } catch (error) {
